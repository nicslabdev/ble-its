...
{
    print_log("**RouterContext::RouterContext**");
...
{
    print_log("**RouterContext::log_packet_drop**");
...
{
    print_log("**RouterContext::set_link_layer**");
...
{
    print_log("**RouterContext::indicate**");
    ...
    if (hdr.source != mib_.itsGnLocalGnAddr.mid() && hdr.type == access::ethertype::GeoNetworking) {
        ...
        printf("The packet is discarded: not GNW or was sended by the device itself\n");
    }
}

void RouterContext::enable(Application* app)
{
    print_log("**RouterContext::enable**");
    app->router_ = &router_;
...
{
    print_log("**RouterContext::disable**");
    if (app->port() != btp::port_type(0)) {
...
{
    print_log("**RouterContext:.require_position_fix**");
...
{
    print_log("**RouterContext:update_position_vector**");
...
{
    print_log("**RouterContext:update_packet_flow**");
