...
public:
    ...
    void build_entity()
    {
        ...
        security::SignService sign_service = straight_sign_service_mod(*cert_provider, *backend, sign_header_policy);
        security::VerifyService verify_service = straight_verify_service_mod(runtime, *cert_provider, cert_validator,
                *backend, cert_cache_v3, cam_store, sign_header_policy, positioning, soft_ver);
    ...
    security::CertificateCache cert_cache_v3; //added
    security::CamStore cam_store; //added
    bool soft_ver; //added
...
create_security_entity(const po::variables_map& vm, const Runtime& runtime, PositionProvider& positioning, bool soft_ver)
{
    ...
    if (name.empty() || name == "none") {
        ...
        std::unique_ptr<SecurityContext> context { new SecurityContext(runtime, positioning, soft_ver) };
