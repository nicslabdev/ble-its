...
void CamApplication::set_interval(Clock::duration interval) //en caso de que se modifique con las opciones a la hora de ejecutar socktap
{
...
void CamApplication::print_generated_message(bool flag) //depende de la opción --print-tx-ca
{
    print_tx_msg_ = flag;
}

void CamApplication::print_received_message(bool flag) //depende de la opción --print-rx-ca
{
    print_rx_msg_ = flag;
}
...
//     may receive the CAM. */
//     request.transport_type = geonet::TransportType::SHB; //The GN Packet transport type parameter specifies the packet transport type (GeoUnicast, SHB, TSB, GeoBroadcast, GeoAnycast). 
//     request.communication_profile = geonet::CommunicationProfile::ITS_G5; //Unspecified, ITS-G5, or LTE-V2X 
//     //The GN Communication profile parameter determines the LL protocol entity (unspecified, ITS-G5A).
