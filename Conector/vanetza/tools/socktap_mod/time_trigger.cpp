...
TimeTrigger::TimeTrigger(asio::io_service& io_service) :
    io_service_(io_service), timer_(io_service), runtime_(Clock::at(now())) //se inicializa el reloj interno
{
...
{
    update_runtime(); //invocar los callbacks expirados
    auto next = runtime_.next();//time point del próximo evento planificado
