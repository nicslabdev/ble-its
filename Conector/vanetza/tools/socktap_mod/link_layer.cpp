...
create_link_layer(boost::asio::io_service& io_service, const EthernetDevice& device, const std::string& name, const std::string& serial_port_tx, const std::string& serial_port_rx)
{
    ...
    if (name == "ethernet" || name == "cohda") {
        ...
        link_layer.reset(new UdpLink { io_service, multicast, serial_port_tx, serial_port_rx});
    }
