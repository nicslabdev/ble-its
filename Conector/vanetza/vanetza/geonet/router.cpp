...
namespace vanetza
{
    namespace geonet
    {
        namespace
        {
            ...
            struct ControlInfo
            {
                ControlInfo(const DataRequest request) : communication_profile(request.communication_profile),
            ...
            template <typename PDU>
            auto create_forwarding_duplicate(const PDU &pdu, const UpPacket &packet) -> std::tuple<std::unique_ptr<ExtendedPdu<typename PDU::ExtendedHeader>>, std::unique_ptr<DownPacket>>
            {
        ...
        Router::Router(Runtime &rt, const MIB &mib) : m_mib(mib),
                                                      m_runtime(rt),
        ...
        {
            print_log("**set_access_interface**");
        ...
        DataConfirm Router::request(const ShbDataRequest &request, DownPacketPtr payload)
        {
            ...
            if (result.accepted())
            {
                ...
                auto transmit = [this, ctrl](PendingPacket::Packet &&packet)
                {
                    ...
                    execute_media_procedures(ctrl.communication_profile); // esto no está implementado (igualmente es Unspecified, no es V2X ni ITS-5)
        ...
        void Router::indicate(UpPacketPtr packet, const MacAddress &sender, const MacAddress &destination)
        {
            ...
            print_log("**Router::indicate**");

            struct indication_visitor : public boost::static_visitor<>
            {
                indication_visitor(Router &router, const IndicationContext::LinkLayer &link_layer, UpPacketPtr packet) : m_router(router), m_link_layer(link_layer), m_packet(std::move(packet))
                {
                }
        ...
        void print_basic_header_info(const BasicHeader *basic)
        {
            printf("Basic Header: \n");
            printf("     - Version %d\n", basic->version.raw());
            printf("     - Next Header: ");
        ...
        {
            print_log("**Router::indicate_basic**");
        ...
        {
            print_log("**Router::indicate_common**");

            // El mensaje que contiene el certificado no tiene ni la cabecera Common ni la Extended
        ...
        void print_secured_info(const IndicationContext::SecuredMessage *m)
        {
            printf("Secured Message:\n");
            printf("     - Protocol version %d\n", m->version);
            printf("     - Header fields \n");

            std::string type;
            for (vanetza::security::HeaderField field : m->header_fields)
            {
                switch (get_type(field))
                {
                case vanetza::security::HeaderFieldType::Generation_Time:
                    type = "Generation Time";
                    break;

                case vanetza::security::HeaderFieldType::Generation_Time_Confidence:
                    type = "Generation Time Confidence";
                    break;

                case vanetza::security::HeaderFieldType::Expiration:
                    type = "Expiration";
                    break;

                case vanetza::security::HeaderFieldType::Generation_Location:
                    type = "Generation Location";
                    break;

                case vanetza::security::HeaderFieldType::Request_Unrecognized_Certificate:
                    type = "Request Unrecognized Certificate";
                    break;

                case vanetza::security::HeaderFieldType::Its_Aid:
                    type = "ITS AID";
                    break;

                case vanetza::security::HeaderFieldType::Signer_Info:
                    type = "Signer Info";
                    break;

                case vanetza::security::HeaderFieldType::Encryption_Parameters:
                    type = "Encryption Parameters";
                    break;

                case vanetza::security::HeaderFieldType::Recipient_Info:
                    type = "Recipient Info";
                    break;
                }
                std::cout << "    * Header field | Type (" + type + ") \n";
            }

            switch (m->payload.type)
            {
            case vanetza::security::PayloadType::Signed:
                type = "     - Payload Type (Signed)";
                break;

            case vanetza::security::PayloadType::Encrypted:
                type = "     - Payload Type (Encrypted)";
                break;

            case vanetza::security::PayloadType::Unsecured:
                type = "     - Payload Type (Unsecured)";
                break;

            case vanetza::security::PayloadType::Signed_And_Encrypted:
                type = "     - Payload Type (Signed and Encrypted)";
                break;

            case vanetza::security::PayloadType::Signed_External:
                type = "     - Payload Type (Signed external)";
                break;

            case vanetza::security::PayloadType::Empty:
                type = "     - Payload Type (Empty)";
                break;

            default:
                type = "     - Payload Type (What?)";
                break;
            }

            printf("%s\n",type.c_str());
            printf("     - Trailer fields \n");
            std::string t_type;
            for (vanetza::security::TrailerField field : m->trailer_fields)
            {
                switch (get_type(field))
                {
                case vanetza::security::TrailerFieldType::Signature:
                    t_type = "Signature";
                    break;
                ...
                std::cout << "          * Trailer field | Type (" + t_type + ")\n";
            }
        ...
        {
            print_log("**Router::indicate_secured**");
            struct secured_payload_visitor : public boost::static_visitor<>
            {
                secured_payload_visitor(Router &router, IndicationContextBasic &ctx, const BasicHeader &basic) : m_router(router), m_context(ctx), m_basic(basic)
                {
                }
            ...
            {
                // Decap packet
                // Para llegar hasta aquí ha sido necesario deserializar el paquete. Ahora está contenido en secured_message

                print_secured_info(secured_message);
                ...
                case DecapReport::Success_Certificate:
                    printf("Success_Certificate\n");
                    break;

                case DecapReport::Signer_Certificate_Not_Found_v3:
                    printf("Signer_Certificate_Not_Found_v3\n");
                    break;

                case DecapReport::Invalid_Certificate:
                    printf("Invalid_Certificate\n");
                    break;
                }
                ...
                }else if(DecapReport::Success_Certificate == decap_confirm.report){
                    printf("Certificate process finished.\n");
                }
                else if (SecurityDecapHandling::Non_Strict == m_mib.itsGnSnDecapResultHandling)
                {
                    ...
                    print_log("**SecurityDecapHandling::Non_Strict**");
                    switch (decap_confirm.report)
                    {
                    ...
                    case DecapReport::Unsigned_Message:
                        break;
                    case DecapReport::Signer_Certificate_Not_Found_v3:
                        // although it's not verified, continue
                        printf("Although it's not verified, continue (soft verfication enabled)\n");
                        print_log("**DecapReport::Signer_Certificate_Not_Found_v3**");
                        boost::apply_visitor(visitor, decap_confirm.plaintext_payload);
                        break;
                    case DecapReport::Signer_Certificate_Not_Found_v2:
                        break;
                    case DecapReport::Unsupported_Signer_Identifier_Type:
                        break;
                    case DecapReport::Unencrypted_Message:
                        // ok, continue
                        print_log("**DecapReport::Unencrypted_Message**");
                        boost::apply_visitor(visitor, decap_confirm.plaintext_payload);
                        break;
                ...
                {
                    // discard packet
                    print_log("**SecurityDecapHandling::Strict**");
                    packet_dropped(PacketDropReason::Decap_Unsuccessful_Strict);
                }
            }
            else
        ...
        void print_extended_info(vanetza::geonet::HeaderConstRefVariant h){
            printf("Extended Header - SHB\n");
            printf("     - Source Position (Long Position Vector)\n");
            printf("         * Address (Mac Address) ");

            const ShbHeader& header = boost::get<const ShbHeader>(h);
            LongPositionVector lpv = header.source_position;

            for(int i = 0; i < lpv.gn_addr.mid().length_bytes; i++){
                if( i != 5UL){
                    printf("%02x:", lpv.gn_addr.mid().octets.at(i));
                }else{
                    printf("%02x \n", lpv.gn_addr.mid().octets.at(i));
                }
            }
            printf("         * Address (Station Type %ld) \n", lpv.gn_addr.station_type());
            printf("         * Address (Country Code %ld) \n", lpv.gn_addr.country_code());
            printf("         * Latitude %d\n", lpv.latitude.value());
            printf("         * Longitude %d\n", lpv.longitude.value());
            printf("         * Position Accuracy Indicator %d\n", lpv.position_accuracy_indicator);
            printf("         * Heading %ld \n", lpv.speed.value());
            printf("         * TimeStamp %ld \n", lpv.timestamp.millisecond());
        }


        void Router::indicate_extended(IndicationContext &ctx, const CommonHeader &common)
        {
            print_log("**Router::indicate_extended**");
            struct extended_header_visitor : public boost::static_visitor<bool>
            {
                extended_header_visitor(Router &router, IndicationContext &ctx, const UpPacket &packet) : m_router(router), m_context(ctx), m_packet(packet)
                {
                }
            ...
            else
            {
                ...
                print_extended_info(extended.get());
        ...
        void Router::execute_itsg5_procedures()
        {
            ...
            print_log("ITS-G5A procedures are not implemented yet \n");
        }
        ...
        void Router::reset_beacon_timer(Clock::duration next_beacon)
        {
            m_runtime.cancel(this);
            m_runtime.schedule(
                next_beacon, [this](Clock::time_point)
