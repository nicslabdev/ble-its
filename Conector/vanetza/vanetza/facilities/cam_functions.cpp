...
void print_indented_mod(std::ostream& os, const asn1::Cam& message, const std::string& indent, unsigned level)
{
    ...
    if (cam.camParameters.highFrequencyContainer.present == HighFrequencyContainer_PR_basicVehicleContainerHighFrequency) {
        ...
        prefix("Acceleration Control") <<bvc.accelerationControl->buf <<"\n";
        prefix("Lane Position")<< *(bvc.lanePosition)<<"\n";
        prefix("Steering Wheel [Confidence]")<<bvc.steeringWheelAngle->steeringWheelAngleValue <<" ["<<bvc.steeringWheelAngle->steeringWheelAngleConfidence<<"]\n";
        prefix("Vertical Acceleration [Confidence]")<<bvc.verticalAcceleration->verticalAccelerationValue <<" ["<<bvc.steeringWheelAngle->steeringWheelAngleConfidence<<"]\n";
        prefix("Performance Class")<<*(bvc.performanceClass)<<"\n";
        prefix("CEN DSRC Tolling Zone id [lat] [lon]")<<*(bvc.cenDsrcTollingZone->cenDsrcTollingZoneID)<<" ["<<bvc.cenDsrcTollingZone->protectedZoneLatitude<<"] ["<<bvc.cenDsrcTollingZone->protectedZoneLongitude<<"]\n";
        --level;
    } else {
    ...
    low_f_containter =  cam.camParameters.lowFrequencyContainer;
    if ((low_f_containter != NULL) && (low_f_containter->present == LowFrequencyContainer_PR_basicVehicleContainerLowFrequency)) {
        prefix("Low Frequency Container [Basic Vehicle]") << "\n";
        ++level;
        prefix("Vechicle Role") << low_f_containter->choice.basicVehicleContainerLowFrequency.vehicleRole <<"\n";
        prefix("Exterior Ligths");
        for(int i=0; i < low_f_containter->choice.basicVehicleContainerLowFrequency.exteriorLights.size; i++){
            printf("%02x ", low_f_containter->choice.basicVehicleContainerLowFrequency.exteriorLights.buf[i]);
        }
        printf("\n");
        prefix("Path History [num]") << low_f_containter->choice.basicVehicleContainerLowFrequency.pathHistory.list.count << "\n";        
        for(int i = 0; i < low_f_containter->choice.basicVehicleContainerLowFrequency.pathHistory.list.count; i++){
            prefix("Path Point [id]")<<i<<"\n";
            prefix("    Path Position [D Alt] [D Lat] [D Lon] ")<<" ["<<low_f_containter->choice.basicVehicleContainerLowFrequency.pathHistory.list.array[i]->pathPosition.deltaAltitude<<"] ["<<low_f_containter->choice.basicVehicleContainerLowFrequency.pathHistory.list.array[i]->pathPosition.deltaLatitude<<"] ["<<low_f_containter->choice.basicVehicleContainerLowFrequency.pathHistory.list.array[i]->pathPosition.deltaLongitude<<"] \n";
            prefix("    Path Delta Time")<<*(low_f_containter->choice.basicVehicleContainerLowFrequency.pathHistory.list.array[i]->pathDeltaTime)<<"\n";
        }
