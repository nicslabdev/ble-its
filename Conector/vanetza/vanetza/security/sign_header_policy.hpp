...
public:
    ...
    /** MODIFICATION FOR SECURED PACKET VERSION 3
     * Prepare header fields for next secured message.
     *
     * \param req signing request (including ITS-AID for example)
     * \param certprvd available certificates
     * \return header fields
     */
    virtual std::list<HeaderField> prepare_header_mod(const SignRequest& req, CertificateProvider& certprvd, int& ret) = 0;
...
public:
    ...
    std::list<HeaderField> prepare_header_mod(const SignRequest& request, CertificateProvider& certificate_provider, int& ret) override;
