...
std::list<HeaderField> DefaultSignHeaderPolicy::prepare_header_mod(const SignRequest& request, CertificateProvider& certificate_provider, int& ret)
{
    print_log("**DefaultSignHeaderPolicy::prepare_header**");
    std::list<HeaderField> header_fields;

    //ORIGINAL header_fields.push_back(convert_time64(m_runtime.now()));
    header_fields.push_back(IntX(request.its_aid));
    ...
    if (request.its_aid == aid::CA) {
        ...
        if (m_chain_requested) {
            ...
            ret = (uint8_t)SignerInfoType::Certificate_Chain;
            m_cam_next_certificate = m_runtime.now() + std::chrono::seconds(1);
            ...
            ret = (uint8_t)SignerInfoType::Certificate_Digest_With_SHA256;
        } else {
            header_fields.push_back(SignerInfo { certificate_provider.own_certificate() });
            m_cam_next_certificate = m_runtime.now() + std::chrono::seconds(1);
            
            ret = (uint8_t)SignerInfoType::Certificate;
        }
