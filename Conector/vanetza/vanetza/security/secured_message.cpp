...
{   
    ...
    if (protocol_version == 2) {
        const size_t hdr_length = deserialize(ar, message.header_fields);
        log = "**Tras deserializar la cabecera he obtenido "+std::to_string(hdr_length)+ " bytes**";
        print_log(log);

        length += hdr_length + length_coding_size(hdr_length);
        log = "**Cabecera + tamaño de cabecera "+std::to_string(length)+" bytes**";
        print_log(log);

        length += deserialize(ar, message.payload);
        log = "**Tras deserializar el payload he obtenido "+std::to_string(length)+" bytes en total**";
        print_log(log);

        const size_t trlr_length = deserialize(ar, message.trailer_fields);
        log = "**Tras deserializar el trailer he obtenido "+std::to_string(trlr_length)+ " bytes**";
        print_log(log);

        length += trlr_length + length_coding_size(trlr_length);
        log = "**Tamaño total "+std::to_string(length)+" bytes**";
        print_log(log);
        ...
        length += deserialize(ar, message.payload);
        log = "**Tras deserializar la el payload he obtenido "+std::to_string(length)+" bytes en total**";
        print_log(log);
        ...
        print_log("**Deserialization_error. Versión del protocolo incorrecta "+ protocol_version);
        throw deserialization_error("Unsupported SecuredMessage protocol version");
    }
    return length;
}
