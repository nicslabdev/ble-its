...
SignService straight_sign_service(CertificateProvider& certificate_provider, Backend& backend, SignHeaderPolicy& sign_header_policy)
{
    return [&](SignRequest&& request) -> SignConfirm {
        print_log("**straight_sign_service**");
        SignConfirm confirm;
        ...
        confirm.secured_message.protocol_version(2);
...
SignService straight_sign_service_mod(CertificateProvider& certificate_provider, Backend& backend, SignHeaderPolicy& sign_header_policy)
{
    return [&](SignRequest&& request) -> SignConfirm {
        print_log("**straight_sign_service_mod**");
        SignConfirm confirm;
        int ret;

        confirm.secured_message.header_fields = sign_header_policy.prepare_header_mod(request, certificate_provider, ret);

        if (ret == (uint8_t)SignerInfoType::Certificate_Digest_With_SHA256){
            printf("The frame includes a hash\n");
            confirm.secured_message.protocol_version(3);
            confirm.secured_message.payload.type = PayloadType::Signed;
            confirm.secured_message.payload.data = std::move(request.plain_message);
        ...
        }else if(ret == (uint8_t)SignerInfoType::Certificate){
            printf("The frame includes a certificate\n");
            confirm.secured_message.protocol_version(3); 
            confirm.secured_message.payload.type = PayloadType::Empty;
            if(confirm.secured_message.payload.data.empty() == true){
                printf("Empty payload\n");
            }else{
                printf("Not empty payload\n");
            }
            confirm.secured_message.trailer_fields.clear();

        }else if(ret == (uint8_t)SignerInfoType::Certificate_Chain){
            printf("The frame includes a certificate chain\n");
            //por ahora no hemos tratado este caso
        }
